...
# ─── COMPONENT IDENTITY MAPPER V6 ───────────────────────────────────────────
# READ-ONLY diagnostika.
# Cíl: najít AKTUÁLNÍ identitu komponent (Read Value), ne Ex-factory Value.
# Mapper prochází Lockdown + DiagnosticsService a mapuje:
# source -> method/domain -> JSON path -> raw value -> známá SN / kandidát.

COMPONENT_V6_KNOWN = {
    "rear_camera": ["GCF918708MRHYMX5U"],
    "front_camera": ["DNM83850UJWJH834W"],
    "battery": ["F5D4277HSTZYT0QA8"],
    "device_or_board": ["F8Y83860U9GJT0Q02", "F3X8405016AJVN7A"],
    "unknown_reference": ["DN883779V9YHYMX5T"],
}

COMPONENT_V6_KEYWORDS = [
    "serial", "sn", "camera", "rear", "front", "wide", "tele", "ultra",
    "display", "screen", "lcd", "panel", "lcm", "touch", "mesa", "pearl",
    "face", "truedepth", "battery", "gasgauge", "mlb", "board",
    "logicboard", "baseband", "nand", "wifi", "bluetooth", "component",
    "part", "factory", "manufact", "calibration", "syscfg", "fdr",
]

COMPONENT_V6_DOMAINS = [
    None,
    "com.apple.disk_usage",
    "com.apple.disk_usage.factory",
    "com.apple.mobile.battery",
...
COMPONENT_V6_IOREG_ENTRIES = [
    "AppleSmartBattery",
    "AppleARMPMUCharger",
    "AppleGasGauge",
    "AppleEmbeddedBattery",
    "AppleH10CamIn",
    "AppleH10CamIn2",
    "AppleH10CamIn3",
    "AppleH10PearlCam",
...
def _component_v6_safe(value, depth=0):
    if depth > 14:
        return "<max-depth>"
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    ...
    if isinstance(value, dict):
        return {str(k): _component_v6_safe(v, depth + 1)
                for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_component_v6_safe(v, depth + 1) for v in value]
    return str(value)

def _component_v6_walk(value, path="$"):
    yield path, "", value
    if isinstance(value, dict):
        for key, child in value.items():
            child_path = f"{path}.{key}"
            yield child_path, str(key), child
            yield from _component_v6_walk(child, child_path)
    elif isinstance(value, (list, tuple)):
        for idx, child in enumerate(value):
            child_path = f"{path}[{idx}]"
            yield child_path, str(idx), child
...
def _component_v6_scalar_variants(value):
    out = []
    if isinstance(value, str):
        out.append(("text", value))
    elif isinstance(value, (int, float, bool)):
        out.append(("scalar", str(value)))
    elif isinstance(value, (bytes, bytearray)):
        raw = bytes(value)
        out.append(("hex", raw.hex()))
...
def _component_v6_classify(path, key):
    s = f"{path} {key}".lower()
    labels = []
    rules = {
        "rear_camera": ("rear", "wide", "tele", "ultra", "cam", "camera"),
        "front_camera": ("front", "pearl", "truedepth"),
        "screen": ("display", "screen", "lcd", "panel", "lcm"),
        "battery": ("battery", "gasgauge"),
        "mainboard": ("mlb", "logicboard", "board", "baseband", "nand"),
        "biometric": ("mesa", "pearl", "touchid", "faceid"),
    }
...
def _component_v6_analyze(source, value, known):
    exact_hits = []
    candidates = []
    known_flat = []
    for component, needles in known.items():
        for needle in needles:
            known_flat.append((component, str(needle).upper()))

    for path, key, child in _component_v6_walk(value):
        variants = _component_v6_scalar_variants(child)
        key_match = any(word in key.lower() for word in COMPONENT_V6_KEYWORDS)
        path_match = any(word in path.lower() for word in COMPONENT_V6_KEYWORDS)

        for encoding, scalar in variants:
            upper = scalar.upper()
            for component, needle in known_flat:
            ...
            # SN-like ASCII candidate. Schválně široké, ale jen na relevantních cestách.
            compact = "".join(ch for ch in scalar if ch.isalnum())
            sn_like = 8 <= len(compact) <= 32 and compact.isalnum()
            if (key_match or path_match) and (sn_like or isinstance(child, (bytes, bytearray))):
                candidates.append({
                    "component_guess": _component_v6_classify(path, key),
                    "source": source,
                    "path": path,
                    "key": key,
                    "encoding": encoding,
    ...
    return exact_hits, candidates

async def _component_v6_collect(udid, extra_needles=None):
    import inspect
    import time
    import pymobiledevice3
    from pymobiledevice3.services.diagnostics import DiagnosticsService

    known = {k: list(v) for k, v in COMPONENT_V6_KNOWN.items()}
    for value in (extra_needles or []):
        value = str(value).strip()
        if value:
            known.setdefault("user_needle", []).append(value)
    ...
    async def add_source(source, producer):
        started = time.perf_counter()
        try:
            value = producer()
            if inspect.isawaitable(value):
                value = await value
            safe = _component_v6_safe(value)
            sources[source] = safe
            calls.append({
                "source": source,
                "ok": True,
    ...
    for domain in COMPONENT_V6_DOMAINS:
        source = "lockdown:default" if domain is None else f"lockdown:{domain}"
        if domain is None:
            await add_source(source, lambda: ld.all_values)
        else:
            await add_source(source, lambda d=domain: ld.get_value(domain=d))
    ...
    try:
        ...
        mg = getattr(diag, "mobilegestalt", None)
        if callable(mg):
            await add_source(
                "diagnostics:mobilegestalt:component_keywords",
                lambda: mg(COMPONENT_V6_KEYWORDS),
            )

        ioreg_entry = getattr(diag, "ioregistry_entry", None)
        if callable(ioreg_entry):
            for entry in COMPONENT_V6_IOREG_ENTRIES:
                await add_source(
                    f"diagnostics:ioregistry_entry:{entry}",
                    lambda e=entry: ioreg_entry(e),
                )

        ioreg = getattr(diag, "ioregistry", None)
        if callable(ioreg):
            await add_source("diagnostics:ioregistry:default", lambda: ioreg())
            try:
                sig = inspect.signature(ioreg)
                if "plane" in sig.parameters:
                    await add_source(
                        "diagnostics:ioregistry:IOService",
                        lambda: ioreg(plane="IOService"),
                    )
                    await add_source(
                        "diagnostics:ioregistry:IODeviceTree",
                        lambda: ioreg(plane="IODeviceTree"),
                    )
            except Exception as exc:
                errors["diagnostics:ioregistry:signature"] = (
    ...
    candidates = []
    for source, value in sources.items():
        hits, found = _component_v6_analyze(source, value, known)
        exact_hits.extend(hits)
        candidates.extend(found)

    hit_dedup = {
        (x["component"], x["source"], x["path"], x["needle"], x["encoding"]): x
        for x in exact_hits
    }
    candidate_dedup = {
        (x["source"], x["path"], x["encoding"], x["value"]): x
        for x in candidates
    }

    exact_hits = list(hit_dedup.values())
    candidates = list(candidate_dedup.values())

    by_component = {}
    for hit in exact_hits:
        by_component.setdefault(hit["component"], []).append(hit)

    return {
        "ok": True,
        "probe": "component-identity-mapper-v6",
        "goal": "map current component Read Value source/path",
        "udid": udid,
        "pymobiledevice3_version": getattr(pymobiledevice3, "__version__", "?"),
        "known_needles": known,
        "summary": {
            "sources_read": len(sources),
            "sources_failed": len(errors),
            ...
            "components_hit": sorted(by_component.keys()),
        },
...
@app.route("/api/component-probe/<udid>", methods=["GET", "POST"])
def api_component_probe(udid):
    data = request.get_json(silent=True) or {}
    query_needles = request.args.getlist("needle")
    body_needles = data.get("needles") if isinstance(data.get("needles"), list) else []
    extra_needles = query_needles or body_needles
    try:
        result = _run_async_isolated(
            _component_v6_collect(udid, extra_needles),
            timeout=300,
        )
        return jsonify(result), 200
    except Exception as exc:
...
@app.route("/api/component-probe/<udid>/report", methods=["GET"])
def api_component_probe_report(udid):
    extra_needles = request.args.getlist("needle")
    try:
        result = _run_async_isolated(
            _component_v6_collect(udid, extra_needles),
            timeout=300,
        ...
        result.pop("sources", None)
        # Report držíme čitelný: kandidáti max 1000.
        result["candidates"] = result.get("candidates", [])[:1000]
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({
            "ok": False,
...
@app.route("/api/diag-version", methods=["GET"])
def api_diag_version():
    return jsonify({
        "ok": True,
        "build": "component-identity-mapper-v6",
        "goal": "map current component Read Value source/path",
        "endpoints": [
            "/api/component-probe/<udid>",
            "/api/component-probe/<udid>/report",
        ],
    }), 200
...
# ─── CAMERA / DISPLAY BINARY PROPERTY EXTRACTOR V7 ──────────────────────────
# READ-ONLY forenzní dump cílených IORegistry uzlů.
# Cíl: zachovat raw bytes a analyzovat binární payloady bez depth limitu.

COMPONENT_V7_TARGETS = [
    "AppleH10CamIn",
    "AppleH10PearlCam",
    "AppleCLCD",
    "AppleM2ScalerCSCDriver",
...
COMPONENT_V7_NEEDLES = [
    "GCF918708MRHYMX5U",   # rear camera Current Read Value
    "DNM83850UJWJH834W",   # front camera Current Read Value
]

COMPONENT_V7_SERIAL_MIN = 8
COMPONENT_V7_SERIAL_MAX = 32

def _component_v7_json_safe(value):
    """Bezztrátová JSON reprezentace hodnot relevantních pro dump."""
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    ...
    if isinstance(value, dict):
        return {str(k): _component_v7_json_safe(v) for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_component_v7_json_safe(v) for v in value]
    return {"__type__": type(value).__name__, "__repr__": repr(value)}

def _component_v7_walk(value, path="$"):
    """Iterativní walk: žádný vlastní depth limit."""
    stack = [(path, "", value)]
    seen = set()
    while stack:
        cur_path, key, cur = stack.pop()
        yield cur_path, key, cur

        if isinstance(cur, (dict, list, tuple, set)):
            oid = id(cur)
            if oid in seen:
                continue
            seen.add(oid)
        ...
        if isinstance(cur, dict):
            ...
            for k, child in reversed(items):
                stack.append((f"{cur_path}.{k}", str(k), child))
        elif isinstance(cur, (list, tuple)):
            for idx in range(len(cur) - 1, -1, -1):
                stack.append((f"{cur_path}[{idx}]", str(idx), cur[idx]))
        elif isinstance(cur, set):
            for idx, child in reversed(list(enumerate(cur))):
                stack.append((f"{cur_path}[{idx}]", str(idx), child))

def _component_v7_printable_runs(text, min_len=6):
    runs = []
    start = None
    for i, ch in enumerate(text):
        ok = ch.isprintable() and ch not in "\r\n\t"
...
def _component_v7_ascii_runs(raw, min_len=6):
    runs = []
    start = None
    for i, b in enumerate(raw):
        ...
        if ok and start is None:
            ...
            if i - start >= min_len:
                runs.append((start, raw[start:i].decode("ascii", errors="ignore")))
            start = None
    if start is not None and len(raw) - start >= min_len:
        runs.append((start, raw[start:].decode("ascii", errors="ignore")))
    return runs

def _component_v7_serial_candidates(text):
    import re
    # Apple serialy se v různých generacích liší délkou; záměrně širší rozsah.
    pattern = rf"(?<![A-Z0-9])[A-Z0-9]{{{COMPONENT_V7_SERIAL_MIN},{COMPONENT_V7_SERIAL_MAX}}}(?![A-Z0-9])"
    out = []
    for m in re.finditer(pattern, text.upper()):
        token = m.group(0)
        if any(c.isalpha() for c in token) and any(c.isdigit() for c in token):
            out.append({"char_offset": m.start(), "value": token})
    return out

def _component_v7_bcd_like(raw):
    """Best-effort nibble/BCD pohled; nic nedekóduje destruktivně."""
    digits = "0123456789ABCDEF"
    return "".join(digits[b >> 4] + digits[b & 0x0f] for b in raw)

def _component_v7_analyze_blob(source, path, key, raw, needles):
    views = []
    hits = []
    candidates = []

    # 1) ASCII sliding/runs
    for offset, value in _component_v7_ascii_runs(raw):
        views.append({"encoding": "ascii", "byte_offset": offset, "value": value[:8000]})
        for cand in _component_v7_serial_candidates(value):
            candidates.append({
                "encoding": "ascii",
                "byte_offset": offset + cand["char_offset"],
                "value": cand["value"],
            })

    # 2) UTF-16 LE/BE v obou byte alignmentech
    for enc in ("utf-16-le", "utf-16-be"):
        for alignment in (0, 1):
            ...
            try:
                ...
                continue
            for char_offset, value in _component_v7_printable_runs(decoded):
                byte_offset = alignment + char_offset * 2
                views.append({
                    "encoding": enc,
                    "alignment": alignment,
                ...
                for cand in _component_v7_serial_candidates(value):
                    candidates.append({
                        "encoding": enc,
                        "alignment": alignment,
                        "byte_offset": byte_offset + cand["char_offset"] * 2,
                        "value": cand["value"],
                    })

    # 3) Hex + BCD-like pohled
    hex_view = raw.hex()
    bcd_view = _component_v7_bcd_like(raw)
    views.append({"encoding": "hex", "byte_offset": 0, "value": hex_view})
    views.append({"encoding": "bcd-like", "byte_offset": 0, "value": bcd_view})

    # Exact needles: plaintext, UTF-16 LE/BE i hex reprezentace.
    for needle in needles:
        needle = str(needle).strip()
        if not needle:
            continue
        variants = [
            ("ascii-bytes", needle.encode("ascii", errors="ignore")),
            ("utf-16-le-bytes", needle.encode("utf-16-le")),
            ("utf-16-be-bytes", needle.encode("utf-16-be")),
        ]
        for encoding, encoded in variants:
            start = 0
            while encoded:
                idx = raw.upper().find(encoded.upper(), start)
                if idx < 0:
                    break
                hits.append({
                    "needle": needle.upper(),
        ...
        hex_needle = needle.encode("ascii", errors="ignore").hex()
        idx = hex_view.lower().find(hex_needle.lower())
        if idx >= 0:
            hits.append({
                "needle": needle.upper(),
                "encoding": "hex-ascii",
    ...
    # Dedup kandidátů a view, aby report zůstal použitelný.
    candidates = list({
        (x.get("encoding"), x.get("byte_offset"), x.get("value")): x
        for x in candidates
    }.values())
    views = list({
        (x.get("encoding"), x.get("alignment"), x.get("byte_offset"), x.get("value")): x
        for x in views
    }.values())
...
def _component_v7_collect_blobs(source, value, needles):
    blobs = []
    for path, key, child in _component_v7_walk(value):
        if isinstance(child, (bytes, bytearray, memoryview)):
            blobs.append(
                _component_v7_analyze_blob(source, path, key, bytes(child), needles)
            )
    return blobs

async def _component_v7_collect(udid, extra_needles=None):
    import inspect
    import time
    import pymobiledevice3
    from pymobiledevice3.services.diagnostics import DiagnosticsService

    needles = list(COMPONENT_V7_NEEDLES)
    for value in (extra_needles or []):
        value = str(value).strip()
        if value and value.upper() not in {x.upper() for x in needles}:
    ...
    async def read_entry(entry):
        started = time.perf_counter()
        source = f"diagnostics:ioregistry_entry:{entry}"
        try:
            method = getattr(diag, "ioregistry_entry", None)
            if not callable(method):
                raise AttributeError("DiagnosticsService.ioregistry_entry unavailable")
            value = method(entry)
            if inspect.isawaitable(value):
                value = await value
            nodes[entry] = _component_v7_json_safe(value)
            found = _component_v7_collect_blobs(source, value, needles)
            blobs.extend(found)
            calls.append({
                "source": source,
                "ok": True,
                "result_type": type(value).__name__,
                "blob_count": len(found),
                "elapsed_ms": round((time.perf_counter() - started) * 1000, 2),
            })
        except Exception as exc:
            msg = f"{type(exc).__name__}: {exc}"
    ...
    for entry in COMPONENT_V7_TARGETS:
        await read_entry(entry)

    # Full IOService tree slouží k dohledání rodičů/dětí targetů.
    # Neanalyzujeme naslepo všechny bloby; vyřízneme jen větve/cesty s target názvem.
    tree_source = "diagnostics:ioregistry:IOService"
    try:
        ioreg = getattr(diag, "ioregistry", None)
        if callable(ioreg):
            started = time.perf_counter()
            try:
                sig = inspect.signature(ioreg)
                value = ioreg(plane="IOService") if "plane" in sig.parameters else ioreg()
            except (TypeError, ValueError):
                value = ioreg()
            if inspect.isawaitable(value):
                value = await value

            target_lower = [x.lower() for x in COMPONENT_V7_TARGETS]
            matched_subtrees = []
            for path, key, child in _component_v7_walk(value):
                identity = f"{path} {key}"
                if isinstance(child, dict):
                    for id_key in ("name", "IOClass", "className", "IOProviderClass"):
                        if id_key in child:
                            identity += " " + str(child.get(id_key))
                if any(t in identity.lower() for t in target_lower):
                    matched_subtrees.append({
                        "path": path,
                        "value": _component_v7_json_safe(child),
                    })
                    blobs.extend(_component_v7_collect_blobs(
                        f"{tree_source}:{path}", child, needles
                    ))

            nodes["__IOService_target_subtrees__"] = matched_subtrees
            calls.append({
                "source": tree_source,
                "ok": True,
                "matched_subtrees": len(matched_subtrees),
                "elapsed_ms": round((time.perf_counter() - started) * 1000, 2),
            })
    except Exception as exc:
        errors[tree_source] = f"{type(exc).__name__}: {exc}"

    # Dedup blobů podle zdroje/cesty/raw payloadu.
    blobs = list({
        (b["source"], b["path"], b["raw_hex"]): b
        for b in blobs
    }.values())
    ...
    for blob in blobs:
        ...
        for cand in blob["serial_candidates"]:
    ...
    serial_candidates = list({
        (x["source"], x["path"], x["encoding"], x["byte_offset"], x["value"]): x
        for x in serial_candidates
    }.values())

    return {
        "ok": True,
        "probe": "camera-display-binary-property-extractor-v7",
        "goal": "dump targeted camera/display IORegistry binary properties and locate Current Read Value",
        "udid": udid,
        "pymobiledevice3_version": getattr(pymobiledevice3, "__version__", "?"),
        "targets": COMPONENT_V7_TARGETS,
        "needles": needles,
        "summary": {
            "targets_requested": len(COMPONENT_V7_TARGETS),
            "calls_total": len(calls),
            "calls_failed": sum(1 for x in calls if not x.get("ok")),
            "binary_blobs": len(blobs),
            "binary_bytes_total": sum(x["bytes_len"] for x in blobs),
            "exact_hits": len(exact_hits),
            "serial_candidates": len(serial_candidates),
        },
        "exact_hits": exact_hits,
...
@app.route("/api/component-binary-probe/<udid>", methods=["GET", "POST"])
def api_component_binary_probe_v7(udid):
    data = request.get_json(silent=True) or {}
    query_needles = request.args.getlist("needle")
    body_needles = data.get("needles") if isinstance(data.get("needles"), list) else []
    extra_needles = query_needles or body_needles
    try:
        result = _run_async_isolated(
            _component_v7_collect(udid, extra_needles),
            timeout=300,
        )
        return jsonify(result), 200
    except Exception as exc:
...
@app.route("/api/component-binary-probe/<udid>/report", methods=["GET"])
def api_component_binary_probe_v7_report(udid):
    extra_needles = request.args.getlist("needle")
    try:
        result = _run_async_isolated(
            _component_v7_collect(udid, extra_needles),
        ...
        # Report ponechá analýzu blobů, ale vyhodí duplicitní plný node dump.
