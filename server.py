...
# ─── COMPONENT REGISTRY V4 – EX-FACTORY RAW BLOB PROBE ──────────────────────
# READ-ONLY diagnostika. Cíl: najít známé factory/current SN i uvnitř raw bytes,
# plistů, base64, gzip/zlib/lzma blobů a v ASCII/UTF-8/UTF-16 reprezentaci.

COMPONENT_PROBE_GROUPS = {
    "mainboard": ["mlb", "logicboard", "logic board", "board", "baseband", "syscfg"],
    "battery": ["battery", "cycle", "capacity", "gasgauge", "gas gauge", "batt", "mfgdata", "manufacturerdata"],
    "rear_camera": ["rear", "backcamera", "back camera", "widecamera", "telephoto", "camera"],
    "front_camera": ["front", "truedepth", "true depth", "pearl", "camera"],
    "display": ["display", "lcd", "screen", "panel", "lcm", "dcp"],
    "nand": ["nand", "storage", "flash", "ans2", "nvme"],
    "wifi_bluetooth": ["wifi", "wi-fi", "wlan", "bluetooth", "macaddress", "mac address", "wireless"],
    "biometrics": ["mesa", "pearl", "savage", "yonkers", "faceid", "face id", "touchid", "touch id"],
    "calibration": ["calibration", "factory", "manufact", "fdr", "gauss"],
}

def _component_probe_group(path, key):
    hay = f"{path} {key}".lower()
    groups = []
    for group, words in COMPONENT_PROBE_GROUPS.items():
        if any(word in hay for word in words):
            groups.append(group)
    return groups or ["other"]

def _component_probe_inventory(obj):
    names = []
    for name in dir(obj):
...
def _v4_json_safe(value, depth=0):
    if depth > 20:
        return "<max-depth>"
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, (bytes, bytearray)):
        raw = bytes(value)
        return {"__bytes_len__": len(raw), "__bytes_hex__": raw.hex()}
    if isinstance(value, dict):
        return {str(k): _v4_json_safe(v, depth + 1) for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_v4_json_safe(v, depth + 1) for v in value]
    return str(value)

def _v4_walk(value, path="$"):
    if isinstance(value, dict):
        for key, child in value.items():
            child_path = f"{path}.{key}"
            yield child_path, str(key), child
            yield from _v4_walk(child, child_path)
    elif isinstance(value, (list, tuple)):
        for idx, child in enumerate(value):
            child_path = f"{path}[{idx}]"
            yield child_path, str(idx), child
...
def _v4_needle_bytes(needle):
    n = str(needle).strip()
    out = []
    for enc in ("ascii", "utf-8", "utf-16-le", "utf-16-be"):
        try:
            out.append((enc, n.encode(enc)))
        except Exception:
            pass
    return out

def _v4_printable_strings(raw, min_len=6):
    found, buf = [], bytearray()
    for b in raw:
        if 32 <= b <= 126:
            buf.append(b)
            ...
            if len(buf) >= min_len:
                found.append(bytes(buf).decode("ascii", errors="ignore"))
            buf.clear()
    if len(buf) >= min_len:
        found.append(bytes(buf).decode("ascii", errors="ignore"))
    return found

def _v4_expand_blob(raw, max_depth=3):
    import base64, gzip, zlib, lzma, plistlib
    queue_items = [("raw", bytes(raw), 0)]
    seen = set()
    results = []

    while queue_items:
        label, data, depth = queue_items.pop(0)
        sig = (len(data), data[:64], data[-64:] if len(data) >= 64 else data)
        if sig in seen:
            continue
        seen.add(sig)
        results.append((label, data))
        if depth >= max_depth or len(data) > 16 * 1024 * 1024:
            continue
        ...
        for name, fn in decoders:
            try:
                dec = fn(data)
                if dec and dec != data:
                    queue_items.append((f"{label}>{name}", dec, depth + 1))
            except Exception:
                pass
        ...
        try:
            ...
            if len(stripped) >= 8 and len(stripped) % 4 == 0:
                ...
                if dec and dec != data:
                    queue_items.append((f"{label}>base64", dec, depth + 1))
        except Exception:
            pass

        try:
            obj = plistlib.loads(data)
            results.append((f"{label}>plist-object", obj))
        except Exception:
            pass

    return results

def _v4_search_value(source, path, key, value, needles):
    ...
    if isinstance(value, (bytes, bytearray)):
        ...
        return hits, blob_findings

    expanded = _v4_expand_blob(raw)
    for layer, data in expanded:
        if not isinstance(data, (bytes, bytearray)):
            nested_hits, nested_findings = _v4_analyze(
                f"{source}:{layer}", data, needles
            )
            hits.extend(nested_hits)
            blob_findings.extend(nested_findings)
            continue
        ...
        for needle in needles:
            original = str(needle).strip().upper()
            if not original:
                continue
            for enc, needle_bytes in _v4_needle_bytes(original):
                if needle_bytes.upper() in upper:
                    hits.append({
                        "source": source, "path": path, "key": key,
                        "needle": original, "layer": layer,
                        "matched_as": enc, "blob_len": len(data),
                    })
                needle_hex = needle_bytes.hex().upper()
                if needle_hex and needle_hex in hex_upper:
                    hits.append({
                        "source": source, "path": path, "key": key,
                        "needle": original, "layer": layer,
                        "matched_as": f"{enc}-hex", "blob_len": len(data),
                    })

        strings = _v4_printable_strings(data)
        interesting = [
            s for s in strings
            if len(s) >= 10 and (
        ...
        if interesting:
            blob_findings.append({
                "source": source, "path": path, "key": key,
                "layer": layer, "blob_len": len(data),
                "printable_candidates": interesting[:200],
            })

    return hits, blob_findings

def _v4_analyze(source, raw_value, needles):
    exact_hits = []
    findings = []
    candidates = []

    for path, key, child in _v4_walk(raw_value):
        hits, child_findings = _v4_search_value(source, path, key, child, needles)
        exact_hits.extend(hits)
        findings.extend(child_findings)

        hay = f"{path} {key}".lower()
        if any(word in hay for words in COMPONENT_PROBE_GROUPS.values() for word in words):
            candidates.append({
                "source": source, "path": path, "key": key,
                "value": _v4_json_safe(child),
            })

    return exact_hits, findings, candidates

async def _v4_call_ioregistry(diag):
    import inspect
    method = getattr(diag, "ioregistry", None)
    if method is None:
        raise AttributeError("DiagnosticsService.ioregistry unavailable")
    ...
    for args, kwargs in attempts:
        label = "default" if not kwargs else "_".join(f"{k}={v}" for k, v in kwargs.items())
        try:
            ...
            last_error = exc
    if not results:
        raise last_error or RuntimeError("ioregistry returned no data")
    return results

async def _component_probe_collect(udid, needles):
    ld = await _create_lockdown_for_udid(udid)
    sources_raw, errors = {}, {}
    method_inventory = {"lockdown": _component_probe_inventory(ld)}
    ...
    try:
        from pymobiledevice3.services.diagnostics import DiagnosticsService
        diag = DiagnosticsService(ld)
        method_inventory["diagnostics"] = _component_probe_inventory(diag)

        try:
            for label, value in (await _v4_call_ioregistry(diag)).items():
                sources_raw[f"diagnostics:ioregistry:{label}"] = value
        except Exception as exc:
            errors["diagnostics:ioregistry"] = f"{type(exc).__name__}: {exc}"

        for label, method_name in [("battery", "get_battery"), ("wifi", "get_wifi")]:
            source = f"diagnostics:{label}"
            try:
                method = getattr(diag, method_name, None)
                if method is None:
    ...
    exact_hits, raw_blob_findings = [], []
    grouped = {name: [] for name in COMPONENT_PROBE_GROUPS}
    grouped["other"] = []

    for source, raw_value in sources_raw.items():
        hits, findings, candidates = _v4_analyze(source, raw_value, needles)
        exact_hits.extend(hits)
        raw_blob_findings.extend(findings)
        for item in candidates:
            for group in _component_probe_group(item["path"], item["key"]):
                grouped[group].append(item)

    exact_hits = list({
        (x["source"], x["path"], x["needle"], x["layer"], x["matched_as"]): x
        for x in exact_hits
    }.values())
    raw_blob_findings = list({
        (x["source"], x["path"], x["layer"]): x for x in raw_blob_findings
    }.values())
    for group, items in grouped.items():
        grouped[group] = list({
            (x["source"], x["path"]): x for x in items
        }.values())

    return {
        "ok": True,
        "probe": "component-registry-v4-exfactory-raw-blob",
        "udid": udid,
        "needles": needles,
        "summary": {
            "sources_read": len(sources_raw),
            "sources_failed": len(errors),
            "ioregistry_sources": len([k for k in sources_raw if k.startswith("diagnostics:ioregistry:")]),
            "exact_hits": len(exact_hits),
            "raw_blob_findings": len(raw_blob_findings),
            "group_counts": {k: len(v) for k, v in grouped.items()},
        },
        "exact_hits": exact_hits,
        "raw_blob_findings": raw_blob_findings,
        "grouped_candidates": grouped,
        "method_inventory": method_inventory,
        "errors": errors,
        "sources": {k: _v4_json_safe(v) for k, v in sources_raw.items()},
    }
...
def api_component_probe(udid):
    ...
    query_needles = request.args.getlist("needle")
    body_needles = data.get("needles") if isinstance(data.get("needles"), list) else []
    needles = [str(x).strip() for x in (
        query_needles or body_needles or FACTORY_PROBE_DEFAULT_NEEDLES
    ) if str(x).strip()]
    try:
        return jsonify(_run_async_isolated(
            _component_probe_collect(udid, needles), timeout=240
        )), 200
    except Exception as exc:
        return jsonify({"ok": False, "probe": "component-registry-v4-exfactory-raw-blob",
...
def api_component_probe_report(udid):
    needles = request.args.getlist("needle") or FACTORY_PROBE_DEFAULT_NEEDLES
    try:
        result = _run_async_isolated(_component_probe_collect(udid, needles), timeout=240)
        result.pop("sources", None)
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({"ok": False, "probe": "component-registry-v4-exfactory-raw-blob",
...
def api_diag_version():
    return jsonify({
        "ok": True,
        "build": "component-registry-v4-exfactory-raw-blob",
        "endpoints": [
            "/api/factory-probe/<udid>",
            "/api/factory-probe/<udid>/report",
