...
# ─── DIAGNOSTICS / IOREGISTRY API INTROSPECTION PROBE V8 ────────────────────
# READ-ONLY: zjistí přesně, jaké metody a signatury nabízí aktuálně
# nainstalovaná verze pymobiledevice3 pro DiagnosticsService a IORegistry.

def _component_v8_json_safe(value, depth=0):
    if depth > 6:
        return "<max-depth>"
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    ...
    if isinstance(value, dict):
        return {str(k): _component_v8_json_safe(v, depth + 1)
                for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_component_v8_json_safe(v, depth + 1) for v in value]
    return {"__type__": type(value).__name__, "__repr__": repr(value)}

async def _component_v8_collect(udid):
    import inspect
    import pymobiledevice3
    from pymobiledevice3.services.diagnostics import DiagnosticsService
    ...
    result = {
        "ok": True,
        "probe": "diagnostics-ioregistry-api-introspection-v8",
        "udid": udid,
        "pymobiledevice3_version": getattr(pymobiledevice3, "__version__", "?"),
        "diagnostics_class": f"{type(diag).__module__}.{type(diag).__qualname__}",
        "methods": [],
        "ioregistry": {},
        "service": {},
        "errors": {},
    ...
    # Veřejné i relevantní neveřejné atributy.
    for name in sorted(dir(diag)):
        ...
        try:
            ...
            result["methods"].append({
                "name": name,
                "getattr_error": f"{type(exc).__name__}: {exc}",
            })
            continue
        ...
        if callable(obj):
            ...
            try:
                ...
                if source:
                    item["source"] = source[:20000]
            except Exception as exc:
                item["source_error"] = f"{type(exc).__name__}: {exc}"

        result["methods"].append(item)

    # Zvlášť rozpitvej ioregistry.
    ioreg = getattr(diag, "ioregistry", None)
    if callable(ioreg):
        try:
            result["ioregistry"]["signature"] = str(inspect.signature(ioreg))
        except Exception as exc:
            result["ioregistry"]["signature_error"] = f"{type(exc).__name__}: {exc}"
        try:
            result["ioregistry"]["source"] = inspect.getsource(ioreg)
        except Exception as exc:
            result["ioregistry"]["source_error"] = f"{type(exc).__name__}: {exc}"
        try:
            result["ioregistry"]["module"] = inspect.getmodule(ioreg).__name__
        except Exception:
            pass

    # Rozpitvej underlying service/client objekt DiagnosticsService.
    for attr in ("service", "_service", "lockdown", "_lockdown"):
        try:
            obj = getattr(diag, attr, None)
            if obj is None:
                continue
            service_info = {
            ...
            for name in sorted(dir(obj)):
                ...
                if callable(member):
                    ...
                    try:
                        ...
                        if source:
                            entry["source"] = source[:12000]
                    except Exception:
                        pass
                    service_info["methods"].append(entry)
            result["service"][attr] = service_info
        except Exception as exc:
            result["errors"][f"service:{attr}"] = f"{type(exc).__name__}: {exc}"

    # Bezpečný read-only test existující ioregistry metody.
    if callable(ioreg):
        tests = []
        test_calls = [
            ("no_args", (), {}),
            ("plane_IOService", (), {"plane": "IOService"}),
        ]
        for label, args, kwargs in test_calls:
            try:
                value = ioreg(*args, **kwargs)
                if inspect.isawaitable(value):
                    value = await value
                safe = _component_v8_json_safe(value)
                tests.append({
                    "label": label,
                    "ok": True,
                    "result_type": type(value).__name__,
                    "top_level_keys": list(value.keys())[:200] if isinstance(value, dict) else None,
                    "preview": safe,
                })
            except Exception as exc:
                tests.append({
        ...
        result["ioregistry"]["tests"] = tests

    return result

@app.route("/api/component-introspection/<udid>", methods=["GET"])
def api_component_introspection_v8(udid):
    try:
        result = _run_async_isolated(
            _component_v8_collect(udid),
            timeout=300,
        )
        return jsonify(result), 200
    except Exception as exc:
...
@app.route("/api/component-introspection/<udid>/report", methods=["GET"])
def api_component_introspection_v8_report(udid):
    try:
        result = _run_async_isolated(
            _component_v8_collect(udid),
            timeout=300,
        )
        # Report ponechává introspekci, ale zkrátí obří preview IORegistry testů.
        for test in result.get("ioregistry", {}).get("tests", []):
            test.pop("preview", None)
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({
            "ok": False,
