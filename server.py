...
# ─── LOCKDOWN FLOW TRACE V5 – BATTERY / SERVICE DISCOVERY ───────────────────
# READ-ONLY diagnostika.
# Cíl V5: místo dalšího hledání SN v IORegistry zjistit, jaké Lockdown služby
# a metody pymobiledevice3 skutečně otevírá při battery/diagnostics dotazech.
# Z CAP2 víme, že 3uTools používá StartSession s Label="battery"; Label ale není
# název služby. Proto zde trasujeme start_lockdown_service/start_service a
# voláme dostupné read-only diagnostické metody jednotlivě.
...
def _flow_v5_json_safe(value, depth=0):
    if depth > 12:
        return "<max-depth>"
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, (bytes, bytearray)):
        ...
        return {
            ...
            "__bytes_hex_preview__": raw[:4096].hex(),
        }
    if isinstance(value, dict):
        return {str(k): _flow_v5_json_safe(v, depth + 1)
                for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_flow_v5_json_safe(v, depth + 1) for v in value]
    return str(value)

def _flow_v5_inventory(obj):
    out = []
    for name in dir(obj):
...
def _flow_v5_signature(obj, name):
    import inspect
    try:
...
async def _flow_v5_await(value):
    import inspect
    return await value if inspect.isawaitable(value) else value

def _flow_v5_install_trace(ld, trace):
    """Obalí service-start metody konkrétní Lockdown instance."""
    import inspect
    ...
    for method_name in (
        ...
        try:
            original = getattr(ld, method_name, None)
            if original is None or not callable(original):
                continue

            async def _async_wrapper(*args, __orig=original,
                                     __name=method_name, **kwargs):
                item = {
                    "event": __name,
                    "args": _flow_v5_json_safe(args),
                    "kwargs": _flow_v5_json_safe(kwargs),
                }
                trace.append(item)
                try:
                    result = __orig(*args, **kwargs)
                    result = await _flow_v5_await(result)
                    item["ok"] = True
                    item["result_type"] = type(result).__name__
                    return result
                except Exception as exc:
                    item["ok"] = False
            ...
            # Wrapper je async schválně; DiagnosticsService v aktuálních verzích
            # pymobiledevice3 service start awaituje.
            setattr(ld, method_name, _async_wrapper)
        except Exception as exc:
            trace.append({
                "event": "trace_install_error",
                "method": method_name,
                "error": f"{type(exc).__name__}: {exc}",
            })

async def _flow_v5_call(diag, method_name, args, trace):
    import time
    method = getattr(diag, method_name, None)
    if method is None:
        return {
    ...
    item = {
        "method": method_name,
        "signature": _flow_v5_signature(diag, method_name),
        "args": _flow_v5_json_safe(args),
    }
    try:
        value = method(*args)
        value = await _flow_v5_await(value)
        item["ok"] = True
        item["result_type"] = type(value).__name__
        item["result"] = _flow_v5_json_safe(value)
    except Exception as exc:
        item["ok"] = False
        item["error"] = f"{type(exc).__name__}: {exc}"
    item["elapsed_ms"] = round((time.perf_counter() - started) * 1000, 2)
    item["service_trace"] = trace[before:]
    return item

async def _flow_v5_collect(udid):
    import inspect
    import pymobiledevice3
    from pymobiledevice3.services.diagnostics import DiagnosticsService
    ...
    ld = await _create_lockdown_for_udid(udid)
    trace = []
    _flow_v5_install_trace(ld, trace)

    out = {
        "ok": True,
        "probe": "lockdown-flow-trace-v5",
        "goal": "discover battery diagnostics service flow",
        "udid": udid,
        "pymobiledevice3_version": getattr(pymobiledevice3, "__version__", "?"),
        "lockdown_type": type(ld).__name__,
        "lockdown_methods": _flow_v5_inventory(ld),
        "lockdown_signatures": {},
        "diagnostics_methods": [],
        ...
        "service_trace": trace,
        "notes": [
            "CAP2 showed StartSession Label=battery.",
            "Label is a client/session label, not proof of an Apple service name.",
            "This probe traces services opened by pymobiledevice3 read-only diagnostics calls.",
        ],
    }
    ...
    for name in (
        ...
        if hasattr(ld, name):
            out["lockdown_signatures"][name] = _flow_v5_signature(ld, name)

    try:
        diag = DiagnosticsService(ld)
        out["diagnostics_methods"] = _flow_v5_inventory(diag)

        for name in FLOW_V5_METHODS:
            if hasattr(diag, name):
                out["diagnostics_signatures"][name] = _flow_v5_signature(diag, name)

        # Každý pokus samostatně, aby bylo jasné, která metoda otevřela službu.
        for name in ("get_battery", "get_battery_diagnostics", "battery",
                     "get_diagnostics", "get_wifi"):
            if hasattr(diag, name):
                out["calls"].append(
                    await _flow_v5_call(diag, name, (), trace)
                )

        if hasattr(diag, "ioregistry_entry"):
            for entry in FLOW_V5_IOREG_ENTRIES:
                out["calls"].append(
                    await _flow_v5_call(diag, "ioregistry_entry", (entry,), trace)
                )

        if hasattr(diag, "ioregistry"):
            sig = inspect.signature(getattr(diag, "ioregistry"))
            ...
            for args, kwargs in attempts:
                # _flow_v5_call bere positional args; kwargs řešíme zvlášť zde.
                before = len(trace)
                started = time.perf_counter()
                item = {
                    "method": "ioregistry",
                    "signature": _flow_v5_signature(diag, "ioregistry"),
                    "args": _flow_v5_json_safe(args),
                    "kwargs": _flow_v5_json_safe(kwargs),
                }
                try:
                    value = getattr(diag, "ioregistry")(*args, **kwargs)
                    value = await _flow_v5_await(value)
                    item["ok"] = True
                    item["result_type"] = type(value).__name__
                    item["result"] = _flow_v5_json_safe(value)
        ...
        out["diagnostics_init_error"] = f"{type(exc).__name__}: {exc}"

    # Souhrn unikátních service name z args/kwargs trace.
    service_names = []
    for event in trace:
        args = event.get("args", [])
        ...
        if isinstance(args, list):
            for value in args:
                if isinstance(value, str) and (
                    value.startswith("com.apple.") or value.startswith("com.apple.internal.")
                ):
...
@app.route("/api/flow-probe/<udid>", methods=["GET"])
def api_flow_probe(udid):
    try:
        result = _run_async_isolated(_flow_v5_collect(udid), timeout=240)
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({
            "ok": False,
...
@app.route("/api/flow-probe/<udid>/report", methods=["GET"])
def api_flow_probe_report(udid):
    try:
        result = _run_async_isolated(_flow_v5_collect(udid), timeout=240)
        # Report nechává výsledky metod, ale zkrátí obří byte preview.
        for call in result.get("calls", []):
            value = call.get("result")
            if isinstance(value, dict) and "__bytes_hex_preview__" in value:
                value["__bytes_hex_preview__"] = value["__bytes_hex_preview__"][:512]
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({
            "ok": False,
...
def api_diag_version():
    return jsonify({
        "ok": True,
        "build": "lockdown-flow-trace-v5",
        "goal": "battery service/session flow discovery",
        "endpoints": [
            "/api/flow-probe/<udid>",
            "/api/flow-probe/<udid>/report",
        ],
    }), 200
