...
# ─── COMPONENT REGISTRY V3 – IOREGISTRY DEEP PROBE ──────────────────────────
# READ-ONLY diagnostika. Projde DiagnosticsService.ioregistry(), rekurzivne
# dekoduje bytes (ASCII/UTF-8/UTF-16 LE/BE + HEX) a hleda Ex-factory serialy
# i komponentni klice v celem IORegistry stromu.

COMPONENT_PROBE_GROUPS = {
    "mainboard": ["mlb", "logicboard", "logic board", "board", "baseband", "syscfg"],
    "battery": ["battery", "cycle", "capacity", "gasgauge", "gas gauge", "batt"],
...
def _v3_decode_bytes(data):
    """Vrati vsechny smysluplne textove reprezentace bytes."""
    out = []
    ...
    for enc in ("ascii", "utf-8", "utf-16-le", "utf-16-be"):
        try:
            s = raw.decode(enc, errors="ignore").strip("\x00\r\n\t ")
            printable = "".join(ch for ch in s if ch.isprintable())
            if printable and len(printable) >= 4:
                out.append((enc, printable))
        except Exception:
            pass
    # dedup
    seen, result = set(), []
    for enc, value in out:
        marker = (enc, value)
        if marker not in seen:
            seen.add(marker)
            result.append((enc, value))
    return result

def _v3_scalar_views(value):
    """Textove pohledy pouzite pro exact search."""
    if isinstance(value, (bytes, bytearray)):
        return _v3_decode_bytes(value)
    if isinstance(value, (str, int, float, bool)):
        return [("scalar", str(value))]
    return []

def _v3_json_safe(value, depth=0):
    if depth > 20:
        return "<max-depth>"
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, (bytes, bytearray)):
        return {
            ...
            "__bytes_hex__": bytes(value).hex(),
            "__decoded__": [
                {"encoding": enc, "value": val[:4000]}
                for enc, val in _v3_decode_bytes(value)
                if enc != "hex"
            ],
        }
    if isinstance(value, dict):
        return {str(k): _v3_json_safe(v, depth + 1) for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_v3_json_safe(v, depth + 1) for v in value]
    return str(value)

def _v3_walk(value, path="$"):
    if isinstance(value, dict):
        for key, child in value.items():
            child_path = f"{path}.{key}"
            yield child_path, str(key), child
            yield from _v3_walk(child, child_path)
    elif isinstance(value, (list, tuple)):
        for idx, child in enumerate(value):
            child_path = f"{path}[{idx}]"
            yield child_path, str(idx), child
...
def _v3_needle_variants(needle):
    """Serial hleda jako plain text i jako ASCII/UTF16 hex."""
    n = str(needle).strip()
    if not n:
        return []
    variants = [("plain", n.upper())]
    for enc in ("ascii", "utf-8", "utf-16-le", "utf-16-be"):
        try:
            variants.append((f"{enc}-hex", n.encode(enc).hex().upper()))
        except Exception:
            pass
    return variants

def _v3_analyze(source, raw_value, needles):
    exact_hits = []
    candidates = []
    needle_map = {str(n).upper(): _v3_needle_variants(n) for n in needles if str(n).strip()}

    for path, key, child in _v3_walk(raw_value):
        views = _v3_scalar_views(child)

        for original, variants in needle_map.items():
            matched = None
            for view_enc, view_value in views:
                hay = view_value.upper()
                for variant_name, variant in variants:
                    if variant and variant in hay:
                        matched = (view_enc, variant_name, view_value)
                        break
                if matched:
                    break
            if matched:
                exact_hits.append({
                    ...
                    "key": key,
                    "needle": original,
                    "view_encoding": matched[0],
                    "matched_as": matched[1],
                    "value": matched[2][:4000],
                })
        ...
        if any(word in hay for words in COMPONENT_PROBE_GROUPS.values() for word in words):
            candidates.append({
                ...
                "path": path,
                "key": key,
                "value": _v3_json_safe(child),
            })

    return exact_hits, candidates

async def _v3_call_ioregistry(diag):
    """Kompatibilni volani napric verzemi pymobiledevice3."""
    import inspect
    ...
    # Nejdrive bez argumentu – v nekterych verzich vraci cely strom.
    attempts.append(((), {}))

    # Jine verze pouzivaji plane/name/class.
    for kwargs in (
        {"plane": "IOService"},
        {"plane": "IODeviceTree"},
        {"plane": "IOPower"},
...
async def _component_probe_collect(udid, needles):
    ...
    # Lockdown nechavame jako kontrolni/baseline zdroj.
    for domain in FACTORY_PROBE_DOMAINS:
        source = "lockdown:default" if domain is None else f"lockdown:{domain}"
        try:
            value = ld.all_values if domain is None else ld.get_value(domain=domain)
    ...
    try:
        ...
        # V3 hlavni zdroj: cely IORegistry.
        try:
            registry_results = await _v3_call_ioregistry(diag)
            for label, value in registry_results.items():
                sources_raw[f"diagnostics:ioregistry:{label}"] = value
        except Exception as exc:
            errors["diagnostics:ioregistry"] = f"{type(exc).__name__}: {exc}"

        # Realne existujici pomocne metody z inventory V2.
    ...
    for source, raw_value in sources_raw.items():
        hits, candidates = _v3_analyze(source, raw_value, needles)
        exact_hits.extend(hits)
        for item in candidates:
            for group in _component_probe_group(item.get("path", ""), item.get("key", "")):
                grouped[group].append(item)

    # Dedup exact hitu i kandidatu.
    exact_hits = list({
        (x["source"], x["path"], x["needle"], x["matched_as"]): x
        for x in exact_hits
    }.values())
    ...
    sources_safe = {k: _v3_json_safe(v) for k, v in sources_raw.items()}
    ioreg_sources = [k for k in sources_raw if k.startswith("diagnostics:ioregistry:")]

    return {
        "ok": True,
        "probe": "component-registry-v3-ioregistry-deep",
        "udid": udid,
        "needles": needles,
        "summary": {
            "sources_read": len(sources_raw),
            "sources_failed": len(errors),
            "ioregistry_sources": len(ioreg_sources),
            "exact_hits": len(exact_hits),
...
def api_component_probe(udid):
    ...
    try:
        result = _run_async_isolated(
            _component_probe_collect(udid, needles), timeout=180
        )
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({
...
def api_diag_version():
    return jsonify({
        "ok": True,
        "build": "component-registry-v3-ioregistry-deep",
        "endpoints": [
