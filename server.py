...
# READ-ONLY diagnostika komponent. Nesnaží se měnit data zařízení; inventarizuje
# dostupné lockdown/diagnostics hodnoty a seskupí kandidáty podle komponent.

COMPONENT_PROBE_GROUPS = {
    "mainboard": ["mlb", "logicboard", "board", "baseband", "syscfg"],
    "battery": ["battery", "cycle", "capacity", "gasgauge"],
    "rear_camera": ["rear", "backcamera", "camera"],
    "front_camera": ["front", "true depth", "truedepth", "pearl", "camera"],
    "display": ["display", "lcd", "screen", "panel", "lcm"],
    "nand": ["nand", "storage", "flash"],
    "wifi_bluetooth": ["wifi", "wi-fi", "bluetooth", "macaddress", "wireless"],
    "biometrics": ["mesa", "pearl", "savage", "yonkers", "faceid", "touchid"],
    "calibration": ["calibration", "factory", "manufact", "fdr"],
}
...
async def _component_probe_collect(udid, needles):
    ...
    errors = {}
    method_inventory = {
        "lockdown": _component_probe_inventory(ld),
    }
    ...
    try:
        ...
        for label, method_name, args in jobs:
            source = f"diagnostics:{label}"
            try:
                method = getattr(diag, method_name, None)
                if method is None:
    ...
    for group, items in grouped.items():
        dedup = {(x["source"], x["path"]): x for x in items}
        grouped[group] = list(dedup.values())

    return {
        "ok": True,
        "probe": "component-registry-v2",
        "udid": udid,
        "needles": needles,
        "summary": {
            "sources_read": len(sources),
...
def api_component_probe(udid):
    ...
    try:
        result = _run_async_isolated(
            _component_probe_collect(udid, needles), timeout=120
        )
        return jsonify(result), 200
    except Exception as exc:
        return jsonify({
